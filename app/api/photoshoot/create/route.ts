...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check if user has enough credits (1 credit per photoshoot)
    const requiredCredits = 1
    const { data: currentBalance, error: balanceError } = await supabaseAdmin.rpc('get_user_credit_balance', {
      user_uuid: user.id
    })
    ...
    if (isExactReuse && reuseSessionId) {
      const { data: reuseSession } = await supabaseAdmin
    ...
    const { error: creditError } = await supabaseAdmin.rpc('modify_user_credits', {
      user_uuid: user.id,
